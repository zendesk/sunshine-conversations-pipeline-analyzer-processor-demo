...
class Bot {
    ...
    async userMessage(appUser, msgId, msg, nonce) {
        ...
        if((conversationItem.target == process.env.TARGET_BOT) || (msg.includes(process.env.REDIRECTION_KEYWORD))){
        ...
        //If the new target is not the bot
        if(newTarget != process.env.TARGET_BOT){
            console.log("NEED HUMAN");
            //If the target changed and the new target is Zendesk, we need to send the conversation history
            let history = false;
            if(newTarget != conversationItem.target && newTarget == process.env.TARGET_ZENDESK){
                history = true ;
                console.log("Sending history = true");
            }
            //otherwise we simply let the message go to the backend
            if(newTarget != process.env.TARGET_SALESFORCE && newTarget != process.env.TARGET_MARKETING){
                conversationItem.target = newTarget;
                await this.continueMessage(nonce,conversationItem.target, history);
            }else{
                await this.continueMessage(nonce,newTarget, history);
            }
        }
        return messageItem;
    }
    ...
    async sendMessage(appUserId, text){
        ...
        if (this.smooch && appUserId && text) {
            ...
            .sendMessage(appUserId,{
                ...
                type: 'text',
                name: 'Bot',
                avatarUrl: 'https://media.smooch.io/apps/5e3db64460d6b7000f6e4eff/fdUJ3rWdzykX1hDJ4e00xglS/JD-34-512.png'
            })
            .then((response) => {
    ...
    async sendRedirectionMessage(appUserId, target){
        ...
        switch(target){
            case process.env.TARGET_ZENDESK:
                text = process.env.REDIRECTION_KEYWORD + process.env.ZENDESK_KEYWORD + "]";
                break;
            case process.env.TARGET_SALESFORCE:
                text = process.env.REDIRECTION_KEYWORD + process.env.SALESFORCE_KEYWORD + "]";
                break;
            case process.env.TARGET_MARKETING:
                text = process.env.REDIRECTION_KEYWORD + process.env.MARKETING_KEYWORD + "]";
                break;
            default:
                return;
        }
    ...
    analyseUserMessage(msg){
        if (msg.includes(process.env.CONTEXT_KEYWORD)){
            console.log("SKIP ANALYSIS");
            return null;
        }else if (msg.includes(process.env.ZENDESK_KEYWORD)){
            console.log("NEED TECH SUPPORT");
            return process.env.TARGET_ZENDESK;
        }else if(msg.includes(process.env.SALESFORCE_KEYWORD)){
            console.log("CREATE SALES LEAD");
            return process.env.TARGET_SALESFORCE;
        }else if(msg.includes(process.env.MARKETING_KEYWORD)){
            console.log("GENERATE MARKETING TRACK");
            return process.env.TARGET_MARKETING;
        }
    }
